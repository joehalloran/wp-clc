...
class Ldnclc_Plugin_Admin {
	...
	/**
	 * Creates a new custom post type - Pupil Workshop
	 *
	 * @since 	1.0.0
	 * @access 	public
	...
	public static function new_cpt_workshop() {
		...
		$opts['menu_icon']								= 'dashicons-welcome-learn-more';
	...
	/**
	 * Creates a new taxonomy for a custom post type Pupil Workshops
	 *
	 * @since 	1.0.0
	 * @access 	public
	...
	/**
	 * Creates a new custom post type - Teacher CPD Course
	 *
	 * @since 	1.0.0
	 * @access 	public
	 * @uses 	register_post_type()
	 */
	public static function new_cpt_teacher_cpd() {
		$cap_type 	= 'post';
		...
		$opts['can_export']								= TRUE;
		$opts['capability_type']						= $cap_type;
		$opts['description']							= esc_html__( "Teacher CPD course listings for London CLC", 'ldnclc-plugin' );
		$opts['exclude_from_search']					= FALSE;
		$opts['has_archive']							= TRUE;
		$opts['hierarchical']							= FALSE;
		//$opts['map_meta_cap']							= TRUE;
		...
		$opts['labels']['all_items']					= esc_html__( $plural, 'ldnclc-plugin' );
		$opts['labels']['edit_item']					= esc_html__( "Edit {$single}" , 'ldnclc-plugin' );
		$opts['labels']['menu_name']					= esc_html__( "Teacher CPD", 'ldnclc-plugin' );
		$opts['labels']['name']							= esc_html__( $plural, 'ldnclc-plugin' );
		$opts['labels']['name_admin_bar']				= esc_html__( $single, 'ldnclc-plugin' );
		$opts['labels']['new_item']						= esc_html__( "New {$single}", 'ldnclc-plugin' );
		$opts['labels']['not_found']					= esc_html__( "No {$plural} Found", 'ldnclc-plugin' );
		...
		$opts['rewrite']['feeds']						= FALSE;
		$opts['rewrite']['pages']						= TRUE;
		$opts['rewrite']['slug']						= esc_html__( 'teacher-cpd', 'ldnclc-plugin' );
		$opts['rewrite']['with_front']					= FALSE;
		//$opts = apply_filters( 'reflected-cpt-options', $opts );
		register_post_type( strtolower( $cpt_name ), $opts );
	}
	 ...
	 * @uses 	register_taxonomy()
	 */
	public static function new_taxonomy_teacher_cpd() {
		...
		$opts['rewrite']['with_front']					= FALSE;
		//$opts = apply_filters( '', $opts );
		register_taxonomy( $tax_name, 'teacher_cpd', $opts );
	}
